...
__global__ void generate_trap_table_kernel(
    ...
    // 2. Modified: trap_point_jac directly stores k_trap * G
    ECPointJac trap_point_jac;
    point_copy_jac(&trap_point_jac, &kG_jac);

    // 3. Konversi ke affine untuk fingerprint
    ECPoint trap_point_affine;
    jacobian_to_affine(&trap_point_affine, &trap_point_jac);

    // 4. Hitung fingerprint
    uint64_t low64_x = trap_point_affine.x.data[0] | ((uint64_t)trap_point_affine.x.data[1] << 32);
    uint64_t y_parity = (trap_point_affine.y.data[0] & 1) ? 1 : 0;
    uint64_t fp = splitmix64(low64_x ^ y_parity);

    // 5. Simpan ke trap table
...
// KERNEL UNTUK MITM REVERSE SEARCH dengan BigInt scalars
extern "C"
__global__ void mitm_reverse_search_kernel(
    ...
    uint32_t min_selected_scalars, // New parameter
    uint32_t max_selected_scalars, // New parameter
    BigInt* d_result,
    int* d_found_flag
) {
    ...
    // Pilih random subset dari scalar list
    uint32_t num_to_choose;
    // Generate a random number between min_selected_scalars and max_selected_scalars (inclusive)
    // max_selected_scalars is guaranteed to be <= num_scalars
    // min_selected_scalars is guaranteed to be <= max_selected_scalars
    uint32_t range = max_selected_scalars - min_selected_scalars + 1;
    if (range == 0) { // Should not happen if min <= max
        return;
    }
    ...
    // Assuming max num_scalars will not exceed 128 for stack allocation
    uint32_t chosen_indices[128] ;
    BigInt n_step_bi;
    init_bigint(&n_step_bi, 0);

    for (uint32_t i = 0; i < num_to_choose; i++) {
        // Pilih random scalar dari list
        uint32_t scalar_idx;
        bool is_unique;

        // Cari indeks yang unik
        do {
    ...
    // 4. Calculate T - n_step * G
    ECPointJac search_point_jac;
    add_point_jac(&search_point_jac, &const_target_jacobian, &neg_nG_jac);

    // 5. Konversi ke affine untuk fingerprint (from search_point_jac)
    ECPoint nG_affine;
    jacobian_to_affine(&nG_affine, &search_point_jac);
    if (nG_affine.infinity) return;

    // 6. Hitung fingerprint
    uint64_t low64_x = nG_affine.x.data[0] | ((uint64_t)nG_affine.x.data[1] << 32);
    uint64_t y_parity = (nG_affine.y.data[0] & 1) ? 1 : 0;
    uint64_t fp = splitmix64(low64_x ^ y_parity);
    ...
    if (check_bloom_filter(d_trap_bloom_filter, trap_bloom_size, fp)) {
        ...
        if (k_trap > 0) {
            // COLLISION FOUND: T - n_step * G = k_trap * G  => T = (n_step + k_trap) * G
            if (atomicCAS(d_found_flag, 0, 1) == 0) {
                // Konversi k_trap uint64 ke BigInt
                BigInt k_trap_bi;
                init_bigint(&k_trap_bi, 0);
                k_trap_bi.data[0] = (uint32_t)(k_trap & 0xFFFFFFFF);
                k_trap_bi.data[1] = (uint32_t)(k_trap >> 32);
                ...
                BigInt info;
                init_bigint(&info, 0);
                info.data[0] = (uint32_t)idx; // experiment index
...
// KERNEL VERIFIKASI (optional, untuk debugging)
extern "C"
__global__ void verify_collision_kernel(
    ...
    // 3. Negasi kG
    ECPointJac neg_kG_jac;
    negate_point_jac(&neg_kG_jac, &kG_jac);

    // 4. Hitung n_step * G - k_trap * G
    ECPointJac calculated_jac;
    add_point_jac(&calculated_jac, &nG_jac, &neg_kG_jac);

    // 5. Konversi ke affine
    ECPoint calculated_affine;
    jacobian_to_affine(&calculated_affine, &calculated_jac);

    // 6. Bandingkan dengan target
    ECPoint target_affine;
    init_bigint(&target_affine.x, 0);
    init_bigint(&target_affine.y, 0);
    copy_bigint(&target_affine.x, target_pubkey_x);
    copy_bigint(&target_affine.y, target_pubkey_y);
    target_affine.infinity = false;
    ...
    for (int i = 0; i < BIGINT_WORDS; i++) {
        if (calculated_affine.x.data[i] != target_affine.x.data[i]) {
            x_equal = false;
        }
        if (calculated_affine.y.data[i] != target_affine.y.data[i]) {
            y_equal = false;
        }
    }
...
// KERNEL UNTUK PRE-COMPUTE SCALAR SUMS (optimization untuk BigInt)
extern "C"
__global__ void precompute_scalar_sums_kernel(
    const uint32_t* d_scalar_list,  // Array of BigInts
    uint32_t num_scalars,
    uint32_t* d_precomputed_sums,   // Array of BigInts untuk hasil
    uint32_t max_subset_size
) {
    uint64_t idx = (uint64_t)blockIdx.x * blockDim.x + threadIdx.x;
    uint32_t total_combinations = 1 << max_subset_size;
    ...
    // Hitung sum untuk kombinasi ini
    for (uint32_t i = 0; i < max_subset_size; i++) {
        if (idx & (1 << i)) {
            if (i < num_scalars) {
                BigInt current_scalar;
                read_bigint_from_memory(&current_scalar, d_scalar_list, i);

                BigInt temp_sum;
                bigint_add_bigint(&temp_sum, &sum, &current_scalar);
                copy_bigint(&sum, &temp_sum);
            }
        }
    }

    // Simpan hasil ke memory
    uint32_t* result_ptr = &d_precomputed_sums[idx * BIGINT_WORDS];
    for (int i = 0; i < BIGINT_WORDS; i++) {
        result_ptr[i] = sum.data[i];
    }
}
