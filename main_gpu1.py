...
def calculate_optimal_parameters():
    ...
    params = {
        'trap_size': 2**29,  # 16.7 million - larger trap table
        'total_scalars': 25,  # More scalars for more combinations
        'start_exponent': 1,
        'experiments_per_batch': 2**20,  # 16.7 million per batch
...
def main():
    ...
    parser.add_argument('--batches', type=int, default=2**115,
    ...
    try:
        for batch in range(args.batches):
            ...
            if found_flag_host[0]:
                ...
                private_key = (n_step_val + k_trap) % N # MODIFIED LINE
    ...
    if found and private_key_verified:
        # Calculate private key: K = n_step + k_trap mod N (MODIFIED LINE)
        N = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141
        n_step_val = bigint_np_to_int(n_step_bigint)
        ...
        if (n_step_val > 0 and k_trap > 0):
            ...
            with open(filename, "w") as f:
                ...
                f.write(f"Formula: (n_step + k_trap) * G = T\n")
