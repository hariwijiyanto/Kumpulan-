...
__global__ void generate_trap_table_kernel(
    ...
    add_point_jac(&trap_point_jac, &const_target_jacobian, &kG_jac);
...
__global__ void mitm_reverse_search_kernel(
    ...
    // 2. HITUNG N_STEP * G (menggunakan BigInt scalar)
    ECPointJac nG_jac;
    scalar_multiply_jac_precomputed(&nG_jac, &n_step_bi);

    // 3. KONVERSI KE AFFINE UNTUK FINGERPRINT
    ECPoint nG_affine;
    jacobian_to_affine(&nG_affine, &nG_jac);
    ...
    // 5. CEK DI TRAP TABLE (melalui bloom filter dulu)
    if (check_bloom_filter(d_trap_bloom_filter, trap_bloom_size, fp)) {
        ...
        if (k_trap > 0) {
            // COLLISION FOUND: n_step * G = T + k_trap * G
